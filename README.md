# AI Alpha Miner: 基于 Claude 4.6 的多智能体量化因子挖掘系统

AI Alpha Miner 是一个尖端的自动化量化交易因子（Alpha）挖掘平台。它模仿高级量化研究员的工作流，通过 Claude 4.6 Opus 驱动的多智能体协同（Multi-Agent System）与检索增强生成（RAG）技术，实现从学术灵感检索到因子代码生成、再到严苛金融回测的闭环自动化。
...
### 3. RAG 知识引擎
本地 data/rag_docs/ 目录下存储了丰富的量化知识：
...
.
├── agents/                 # 多智能体定义
│   ├── idea_agent.py       # 负责金融逻辑构思
│   ├── factor_agent.py     # 负责编写 Qlib 公式
│   └── eval_agent.py       # 封装 AlphaEval 与反射逻辑
├── core/                   # 核心底层模块
│   ├── alphaeval/          # AlphaEval 评估框架实现
│   ├── llm.py              # Claude 4.6 Opus 接口集成
│   └── rag.py              # ChromaDB 向量检索与文档加载
├── data/rag_docs/          # RAG 原始文档仓库
│   ├── academic/           # 前沿论文提炼
│   ├── alphas/             # 经典因子公式库
│   └── market_meta/        # 真实市场字段元数据
...
```bash
export ClaudeCode_KEY="你的_gptsapi_key"
```
...
### 运行流程说明：
*   **第一步**: RAGModule 自动扫描 data/rag_docs/ 并将新知识向量化到本地 ChromaDB。
...
## 未来路线图 (Roadmap)
- 支持更多数据源（如聚宽 JoinQuant、Tushare）。
- 增加基于强化学习（RL）的因子参数微调模块。
- 支持实盘接口自动对接。

---

## 免责声明
本系统仅用于学术研究与辅助开发，不构成任何投资建议。量化投资有风险，入市需谨慎。建议在模拟盘环境充分验证后再进行实盘操作。
