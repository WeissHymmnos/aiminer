...
Required `.env` keys: at least one of `OPENAI_API_KEY` / `ZHIPU_API_KEY` / `ANTHROPIC_API_KEY` / … (see `.env.example`); `RQ_TOKEN` (or `RQ_USER`/`RQ_PASS`) for RiceQuant backtesting.
