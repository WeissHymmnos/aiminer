...
class AlphaEval:
    def __init__(
        ...
        if not os.path.exists(expanded_path):
            ...
            from aiminer.core.qlib_custom_ops import register_custom_ops
    ...
    def fetch_data(self):
        ...
        noise_mod = (
            f"{__package__}.noise_proc" if __package__ else "aiminer.core.alphaeval.noise_proc"
        )
    ...
    def summary(self):
        logger.info(f"IC: {self.ic}")
        logger.info(f"RankIC: {self.rankic}")
        logger.info(f"RRE: {self.rre}")
        logger.info(f"PFS1: {self.pfs1}")
        logger.info(f"PFS2: {self.pfs2}")
        logger.info(f"Diversity: {self.diversity}")
        logger.info(f"LLM: {self.llm_avg_score}")
