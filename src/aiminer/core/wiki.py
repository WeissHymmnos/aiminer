...
class LLMWiki:
    def __init__(
        ...
        if getattr(self, "disable_chroma", False):
            ...
            self.client = None
            self.wiki_col: VectorStore | None = None
            return
...
# --- Full-Page Query & Retrieval ---
    ...
    def retrieve(
        ...
        if getattr(self, "disable_chroma", False):
            ...
            if not pages:
                return "No matching content in Wiki."
            parts = ["=== LLM WIKI ==="]
            for page in pages:
                updated_short = (
                ...
                body = page.get("body", "")
                parts.append(f"**{page['title']}** (updated {updated_short})")
        ...
        try:
            count = self._call_chroma_with_retry("count", self.wiki_col.count)
            if count == 0:
                return "Wiki has no knowledge entries yet."
            ...
            for doc, meta in zip(results["documents"][0], results["metadatas"][0]):
                ...
                parts.append(f"**{title}** (updated {updated_short})")
                parts.append(
