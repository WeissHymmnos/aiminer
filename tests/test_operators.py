...
class TestOperators(unittest.TestCase):
    def setUp(self):
        ...
        self.evaluator = RiceQuantEval(factor_expressions=[], test_start_date="2020-01-01", test_end_date="2020-01-05")
        # 模拟注入fields
        self.evaluator.raw_data = pd.DataFrame(index=pd.MultiIndex.from_product([self.instruments, self.dates], names=['instrument', 'datetime'])).sort_index()
