...
class AlphaEval:
    ...
    def LLM_scores(
        ...
        from core.llm import get_llm_config, get_provider_base_url

        # Prefer explicit Claude/Anthropic credentials; fall back to auto-detect.
        try:
            cfg = get_llm_config(provider="claude")
        except ValueError:
            cfg = get_llm_config()
        client = OpenAI(
            api_key=cfg["api_key"],
            base_url=get_provider_base_url(cfg["provider"]),
        )
