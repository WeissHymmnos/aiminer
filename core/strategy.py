...
class StrategyBacktester:
    ...
    def run(self, signal_df: pd.DataFrame, label_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        max_dd = float((cumulative / cumulative.cummax() - 1.0).min()) if not cumulative.empty else 0.0
        annualized_return = (
        ...
        metrics = {
            ...
            "gross_exposure": float(positions.abs().sum(axis=1).mean()) if not positions.empty else 0.0,
            "net_exposure": float(positions.sum(axis=1).mean()) if not positions.empty else 0.0,
    ...
    def _build_time_series_positions(self, signal_df: pd.DataFrame) -> pd.DataFrame:
            ...
            for idx, value in series.items():
                ...
                if cfg.direction == "long_flat":
                    ...
                    if cfg.long_threshold is not None and value >= cfg.long_threshold:
                        ...
                        desired = -1.0
                    elif cfg.exit_threshold is not None and abs(value - cfg.exit_threshold) <= 1e-12:
                        desired = 0.0
        ...
        if cfg.max_positions:
            ranked = signal_df.where(positions != 0.0).abs().rank(axis=1, ascending=False, method="first")
    ...
    def _apply_rebalance_and_constraints(self, desired: pd.DataFrame) -> tuple[pd.DataFrame, Dict[str, Any]]:
        cfg = self.config
        ...
        for dt in desired.index:
            candidate = desired.loc[dt].copy() if rebalance.loc[pd.to_datetime(dt)] else prev.copy()
            if cfg.max_positions:
                non_zero = candidate[candidate != 0.0]
                if len(non_zero) > cfg.max_positions:
                    keep = non_zero.abs().sort_values(ascending=False).head(cfg.max_positions).index
                    candidate.loc[~candidate.index.isin(keep)] = 0.0
            for col in candidate.index:
                if prev[col] != 0.0:
                    holding_days[col] += 1
                if candidate[col] != prev[col] and prev[col] != 0.0 and holding_days[col] < cfg.min_holding_days:
