...
export function ManualBacktestPage() {
  ...
  const detail = selectedBacktest.data ?? runMutation.data ?? null;
        ...
        <div className="list">
          {(history.data ?? []).map((entry, index) => (
            <button
              type="button"
              key={index}
              className={`list-row button-reset ${selectedJobId === String(entry.job_id ?? "") ? "selected" : ""}`}
              onClick={() => setSelectedJobId(String(entry.job_id ?? ""))}
            >
              <div>
