...
def generate_market_metadata():
    ...
    output_dir = os.path.join("data/rag_docs/market_meta", year)
    os.makedirs(output_dir, exist_ok=True)
    file_path = os.path.join(output_dir, f"{year}_market_metadata.md")
    ...
    try:
        ...
        sample_hist = ak.stock_zh_a_hist(symbol="000001", period="daily", start_date="20230101", end_date="20230110", adjust="qfq")
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            ...
            f.write("Category: market_meta\n")
            f.write("Region: CN\n")
            f.write("Keywords: market metadata, a-share, futures, data fields\n")
            f.write("Source: akshare\n")
            f.write(f"Generated-At: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            
            f.write("## Overview\n")
            f.write("This document provides metadata about the financial market data available for factor mining.\n")
            ...
            f.write("The standard daily history data for A-shares (forward adjusted `qfq`) contains the following columns:\n")
            for col in columns:
                f.write(f"- `{col}`\n")
            ...
            f.write("When writing Qlib factor code, map these AKShare/standard columns as follows:\n")
            f.write("- `开盘` -> `$open`\n")
            f.write("- `收盘` -> `$close`\n")
            f.write("- `最高` -> `$high`\n")
            f.write("- `最低` -> `$low`\n")
            ...
            f.write("- All factor values must handle `NaN` gracefully (e.g., using Qlib's robust operations).\n")
            f.write("- Prices are adjusted for splits and dividends.\n")
            f.write("- Futures data usually includes Open Interest (`持仓量`) which can be used as a strong predictor.\n")
