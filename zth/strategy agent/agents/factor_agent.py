...
class FactorAgent:
    ...
    """
    # Verified whitelist of operators supported by the RiceQuant matrix engine (rq_eval.py)
    QLIB_OPERATORS = {
...
"""
    ...
    def _validate_qlib_expression(self, expr: str) -> tuple:
        # Basic syntax validation for supported factor expressions.
        ...
        # Check $ fields
        if '$' not in expr:
            return False, "Expression contains no supported field references (e.g., $close)."
    ...
    def __call__(self, state: AlphaMinerState) -> Dict[str, Any]:
        ...
        try:
            ...
            messages = [
                ("system", "You are an expert quant factor engineer. Convert the following mathematical formula into a syntactically correct factor expression for the RiceQuant matrix engine.\n\n"
