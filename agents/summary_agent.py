...
class SummaryAgent:
    ...
    def generate_markdown_report(self, factor_data: Dict[str, Any]) -> str:
        ...
        # Prefer charts produced by the eval pipeline; otherwise plot from returns.
        plot_paths = dict(plot_paths or {})
        if not plot_paths.get("equity"):
            curve_path = self.generate_equity_curve(returns, factor_id)
            if curve_path:
                plot_paths["equity"] = curve_path

        chart_section = "## 3. Backtest Visualization\n\n"
        if plot_paths.get("equity"):
            chart_section += (
                f"### Cumulative Returns (IS/OOS)\n"
                f"![Equity Curve]({plot_paths['equity']})\n\n"
            )
        if plot_paths.get("layers"):
            chart_section += (
                f"### Layered Returns (G1-G5)\n"
                f"![Layered Curves]({plot_paths['layers']})\n\n"
            )
        if not plot_paths:
